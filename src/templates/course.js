  ...
  <Layout>
    <article className="sheet">
      ...
      <div className="sheet__inner">
        <h1 className="sheet__title">{data.datoCmsCourse.name}</h1>
        <p className="sheet__lead">{data.datoCmsCourse.description}</p>
        <div className="sheet__slider">
          <Slider infinite={true} slidesToShow={2} arrows>
            {data.datoCmsCourse.gallery.map(({ fluid }) => (
              <img alt={data.datoCmsCourse.name} key={fluid.src} src={fluid.src} />
            ))}
          </Slider>
        </div>
        <div
